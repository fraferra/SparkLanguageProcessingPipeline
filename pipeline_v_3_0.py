...
class Pipeline:
	...
	def initialize_params(self, params):
		...
		try:
			...
			finalParams["input_folder"] = params["input_folder"]
	...
	def load_data(self):
		...
		"""
		if self.params["input_type"] == "avro":
			if self.params["num_fragments"] == False:
				data = self.sqlContext.read.load("s3a://" + self.params["input_folder"] + "*.avro", "com.databricks.spark.avro").rdd
			else:
				data = load_partial_data(self.params["input_folder"], self.params["num_fragments"], self.sqlContext)
		if self.params["input_type"] == "parquet":
				data = self.sqlContext.read.load("s3a://" + self.params["input_folder"]).rdd
